...
public class DataPlotter : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        wordName = columnList[columnWord];
        ...
        for (var i = 0; i < pointList.Count; i++)
        {
            ...
            String word = System.Convert.ToString(pointList[i][wordName]);

            // add text labels to datapoints
            GameObject label = Instantiate(Labels, new Vector3(x, y, z) * plotScale, Quaternion.identity);
            TextMesh text = label.GetComponent<TextMesh>();
            text.text = word;
            label.transform.name = word + "Label";

            // Assigns original values to dataPointName
            string dataPointName = word;
